...
function getScriptHash(address) {
  ...
  if (address.startsWith('2') || address.startsWith('tb1') || address.startsWith('m') || address.startsWith('n')) {
...
const main = async () => {
  console.log(`Connecting to Electrum server ${electrumProto}://${electrumHost}:${electrumPort}`)
  ...
  try {
    ...
    const scriptHashHistroy = await ecl.blockchainScripthash_getHistory(scriptHash)
    console.log(`Script hash history ${JSON.stringify(scriptHashHistroy, null, 2)}`)
